...
class _CodeFieldState extends State<CodeField> {
  ...
  void _scrollCaretIntoView() {
    if (_codeScroll == null || !_codeScroll!.hasClients) {
      return;
    }
    ...
    // Get the caret offset and height
    ...
    final viewportHeight = _codeScroll!.position.viewportDimension;
    final currentScrollOffset = _codeScroll!.offset;

    // Vertical margin/padding to prevent cursor from hugging the edge
    const verticalPadding = 24.0;

    // Check if caret is above viewport
    if (caretOffset.dy - verticalPadding < currentScrollOffset) {
      final targetScrollOffset = max(0.0, caretOffset.dy - verticalPadding);
      _codeScroll!.jumpTo(targetScrollOffset);
    } 
    // Check if caret is below viewport
    else if (caretOffset.dy + caretHeight + verticalPadding > currentScrollOffset + viewportHeight) {
      final targetScrollOffset = caretOffset.dy + caretHeight + verticalPadding - viewportHeight;
      final maxExtent = _codeScroll!.position.maxScrollExtent;
      _codeScroll!.jumpTo(min(maxExtent, targetScrollOffset));
    }

    // Handle horizontal auto-scroll
    if (_horizontalCodeScroll != null && _horizontalCodeScroll!.hasClients) {
      final viewportWidth = _horizontalCodeScroll!.position.viewportDimension;
      final currentHorizontalScrollOffset = _horizontalCodeScroll!.offset;
      const horizontalPadding = 32.0;

      // Check if caret is left of viewport
      if (caretOffset.dx - horizontalPadding < currentHorizontalScrollOffset) {
        final targetHorizontalOffset = max(0.0, caretOffset.dx - horizontalPadding);
        _horizontalCodeScroll!.jumpTo(targetHorizontalOffset);
      }
      // Check if caret is right of viewport
      else if (caretOffset.dx + horizontalPadding > currentHorizontalScrollOffset + viewportWidth) {
        final targetHorizontalOffset = caretOffset.dx + horizontalPadding - viewportWidth;
        final maxHorizontalExtent = _horizontalCodeScroll!.position.maxScrollExtent;
        _horizontalCodeScroll!.jumpTo(min(maxHorizontalExtent, targetHorizontalOffset));
      }
    }
  }
